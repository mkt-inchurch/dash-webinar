...
const tooltipStyle = {
  ...
  cursor: { fill: 'rgba(255,255,255,0.03)', stroke: '#1F2225' },
};
    ...
    >
      ...
      <Card title="Gasto (R$)">
        <AreaChart data={rows} margin={{ top: 8, right: 12, left: -12, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="spend" stroke={GREEN} strokeWidth={2} fill="url(#gGasto)" />
        </AreaChart>
      </Card>
      ...
      <Card title="Conversões × CPL">
        <ComposedChart data={rows} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
          ...
          <Bar yAxisId="l" dataKey="leads" name="Conversões" fill={GREEN} radius={[4, 4, 0, 0]} maxBarSize={26} />
          <Line yAxisId="r" type="monotone" dataKey="cpl" name="CPL (R$)" stroke={ORANGE} strokeWidth={2} strokeDasharray="5 4" dot={false} />
        </ComposedChart>
      </Card>
      ...
      <Card title="CTR Link (%)">
        <LineChart data={rows} margin={{ top: 8, right: 12, left: -12, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="ctr" stroke={GREEN} strokeWidth={2.5} dot={false} />
        </LineChart>
      </Card>
      ...
      <Card title="Alcance × Frequência">
        <ComposedChart data={rows} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
          ...
          <Tooltip {...tooltipStyle} />
          <Legend wrapperStyle={{ fontSize: 11 }} />
          <Bar yAxisId="l" dataKey="reach" name="Alcance" fill="#2f7d3a" radius={[4, 4, 0, 0]} maxBarSize={26} />
          <Line yAxisId="r" type="monotone" dataKey="freq" name="Frequência" stroke={ORANGE} strokeWidth={2} dot={false} />
        </ComposedChart>
      </Card>
    </motion.div>
  );
